    ...
    *,
    ...
    for item in generators:
        if not isinstance(item, torch.Generator):
            raise ValueError(
                "`generator` list items must be torch.Generator instances."
            )
    ...
    *,
    ...
    if provided_generator is not None:
        if isinstance(provided_generator, list):
            generator_device = (
                provided_generator[0].device.type
                if len(provided_generator) > 0
                else "cpu"
            )
        else:
            generator_device = (
                provided_generator.device.type
                if hasattr(provided_generator, "device")
                else "cpu"
            )
